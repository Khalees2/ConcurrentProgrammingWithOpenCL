/* *****************************************************************************************************************************************
 *  Program Name: main.c                                                                                                                   *
 *  Problem : Parallel Sum Reduction using OpenCL Library                                                                                  *
 *  Created by Salman Khaleel Sab on 22/04/2019                                                                                            *
 *  Note : You need to add kernel file sumReductionGPU.cl to run this program and change the path of kernel file according to your system  *
 *  Code Reference :                                                                                                                       *
...
// initializing values
void initValues (int nWorkItems, long *input, int nWorkGroups, long *output) {
    
    int i;
    
    // input array will have numbers ranging from 1 to num_work_items
    for (i=0; i<nWorkItems; i++)
        input[i] = i+1;
    
    // output array will have the calculated sum values from each work group but, will be initialized with zeros before calculating sum
...
int main () {
    ...
    // Size of vector : first argument
    int size = 262144*512;
    //int size = 1024*1024;
    
    // Size of WorkGroup : second argument
    int sizeWorkGroup = 256;
    ...
    // Main values array
    xInput  = malloc(nWorkItems*sizeof(long));
    
    // Test allocation
    if (xInput == NULL)
        fprintf(stderr,"Malloc failed for xInput array\n");
    
    // Allocate cumulative error array
    sumReduction = malloc(nWorkGroups*sizeof(long));
    
    // Test allocation
    if (sumReduction == NULL)
        fprintf(stderr,"Malloc failed for sumReduction array\n");
    ...
    fileKernel = fopen("/Users/salmank/Documents/SEM2/CP/Assignment_Two/OpenCL/Parallel_SumReduction/Parallel_SumReduction/sumReductionGPU.cl","r");
    if (!fileKernel) {
    ...
    cl_mem inputBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                        nWorkItems*sizeof(long), NULL, &ret);
    if (ret != CL_SUCCESS)
        fprintf(stderr,"Creating Input Buffer failed\n");
    
    cl_mem reductionBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                            nWorkGroups*sizeof(long), NULL, &ret);
    // Write to buffers
    ret = clEnqueueWriteBuffer(command_queue, inputBuffer, CL_TRUE, 0,
                               nWorkItems*sizeof(long), xInput, 0, NULL, NULL);
    ret = clEnqueueWriteBuffer(command_queue, reductionBuffer, CL_TRUE, 0,
                               nWorkGroups*sizeof(long), sumReduction, 0, NULL, NULL);
    
    // Create a program from the kernel source
    cl_program program = clCreateProgramWithSource(context, 1, (const char **)&kernelSource_str,
    ...
    clSetKernelArg(kernel, 2, local_item_size*sizeof(long),NULL);
    ...
    // Execute kernel code
    if (clEnqueueNDRangeKernel(command_queue, kernel, 1, NULL, &global_item_size, &local_item_size,
                               0, NULL, NULL) != CL_SUCCESS)
        fprintf(stderr,"Error in clEnqueueNDRangeKernel\n");
    
    // Read the buffer back to the array
    if (clEnqueueReadBuffer(command_queue, reductionBuffer, CL_TRUE, 0, nWorkGroups*sizeof(long),
                            sumReduction, 0, NULL, NULL) != CL_SUCCESS)
    ...
    printf("Sum calculated Sequentially: %ld\n\n",finalSumSeq);
    printf("Parallel Sum calculated in GPU: %ld\n\n",finalSumGPU);
    printf("Time Taken to build GPU/OpenCL : Wall Clock = %d second %d micro\n\n",second_gpu_init,micro_gpu_init);
    printf("Time taken for one NDRangeKernel call and WorkGroups : Wall Clock = %d second %d micro\n\n",second_gpu,micro_gpu);
